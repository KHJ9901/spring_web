...
public class BoardServiceImp implements BoardService {	
	...
	/*
	 * @Override public String insertBoard(HttpServletRequest req,
	 * HttpServletResponse resp) {
	 * 
	 * DiskFileItemFactory factory = new DiskFileItemFactory();
	 * factory.setDefaultCharset(CHARSET); ServletFileUpload upload = new
	 * ServletFileUpload(factory);
	 * 
	 * Board board = new Board(); AttachFile attachFile = null; FileService
	 * fileService = new FileServiceImpl();
	 * 
	 * try { List<FileItem> items = upload.parseRequest(req);
	 * System.out.println("폼에서 넘어온 개수 :" + items.size()); for(FileItem item : items)
	 * { if(item.isFormField()) { board = getFormPararmeter(item, board); } else {
	 * attachFile = fileService.fileUpload(item); } } } catch (FileUploadException
	 * e) { e.printStackTrace(); }
	 * 
